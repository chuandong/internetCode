...
int main(int argc, char* argv[]) {
    ...
    */
    ...
    while ((opt=getopt_long(argc, argv,"Vc:?h",long_options,&options_index)) != EOF) {
        switch (opt) {
            ...
            case 'c':
                ...
                break;
            case 'V':
                BASLOG(LOG_LEVEL_DEBUG, PROGRAM_VERSION"\n");
                return 0;
            case ':':
            case 'h':
            case '?':
    ...
    BASLOG(LOG_LEVEL_DEBUG,"conffile = %s, optind=%d,argc=%d", conf_file, optind, argc);

    if (optind < argc) {
        BASLOG(LOG_LEVEL_ERROR, "non-option ARGV-elements: ");
        while (optind < argc)
            BASLOG(LOG_LEVEL_ERROR, "%s ", argv[optind++]);
        return 0;
    }
    ...
    */
    ...
    if (sigaction(SIGPIPE, &sa, NULL)) {
        BASLOG(LOG_LEVEL_ERROR, "install sigal handler for SIGPIPE failed");
        return 0;
    }
    ...
    */
    ...
    if (rc == 0)
    	BASLOG(LOG_LEVEL_DEBUG, "make_socket_non_blocking");
    ...
    */
    ...
    BASLOG(LOG_LEVEL_DEBUG, "zaver started.");
    int n;
    int i, fd;
    int time;
    ...
    while (1) {
        ...
        BASLOG(LOG_LEVEL_DEBUG,"wait time = %d", time);
        ...
        for (i = 0; i < n; i++) {
            ...
            if (listenfd == fd) {
                ...
                while(1) {
                    ...
                    BASLOG(LOG_LEVEL_DEBUG,"new connection fd %d", infd);
                    
                    zv_http_request_t *request = (zv_http_request_t *)malloc(sizeof(zv_http_request_t));
                    if (request == NULL) {
                        BASLOG(LOG_LEVEL_ERROR, "malloc(sizeof(zv_http_request_t))");
                        break;
                    }
                ...
                if ((events[i].events & EPOLLERR) ||
                    ...
                    BASLOG(LOG_LEVEL_ERROR, "epoll error fd: %d", r->fd);
                    close(fd);
                    continue;
                }

                BASLOG(LOG_LEVEL_DEBUG,"new data from fd %d", fd);
    ...
    if (threadpool_destroy(tp, 1) < 0) {
        BASLOG(LOG_LEVEL_ERROR, "destroy threadpool failed");
    }
    */
