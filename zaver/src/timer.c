...
static void zv_time_update() {
    ...
    BASLOG(LOG_LEVEL_DEBUG, "in zv_time_update, time = %zu", zv_current_msec);
}
...
int zv_find_timer() {
    ...
    while (!zv_pq_is_empty(&zv_timer)) {
        BASLOG(LOG_LEVEL_DEBUG, "zv_find_timer");
        zv_time_update();
        ...
        BASLOG(LOG_LEVEL_DEBUG, "in zv_find_timer, key = %zu, cur = %zu",
                timer_node->key,
                zv_current_msec);
        time = (time > 0? time: 0);
        break;
...
void zv_handle_expire_timers() {
    BASLOG(LOG_LEVEL_DEBUG, "in zv_handle_expire_timers");
    zv_timer_node *timer_node;
    int rc;

    while (!zv_pq_is_empty(&zv_timer)) {
        BASLOG(LOG_LEVEL_DEBUG, "zv_handle_expire_timers, size = %zu", zv_pq_size(&zv_timer));
        zv_time_update();
        timer_node = (zv_timer_node *)zv_pq_min(&zv_timer);
        check(timer_node != NULL, "zv_pq_min error");
...
void zv_add_timer(zv_http_request_t *rq, size_t timeout, timer_handler_pt handler) {
    ...
    BASLOG(LOG_LEVEL_DEBUG,"in zv_add_timer, key = %zu", timer_node->key);
...
void zv_del_timer(zv_http_request_t *rq) {
    BASLOG(LOG_LEVEL_DEBUG, "in zv_del_timer");
    zv_time_update();
