...
void do_request(void *ptr) {
    ...
    for(;;) {
        ...
        if (n == 0) {   
            // EOF
            BASLOG(LOG_LEVEL_DEBUG, "read return 0, ready to close fd %d, remain_size = %zu", fd, remain_size);
            goto err;
        }

        if (n < 0) {
            if (errno != EAGAIN) {
                BASLOG(LOG_LEVEL_ERROR, "read err, and errno = %d", errno);
                goto err;
            }
            break;
        }
        ...
        BASLOG(LOG_LEVEL_DEBUG, "ready to parse request line"); 
        rc = zv_http_parse_request_line(r);
        if (rc == ZV_AGAIN) {
            continue;
        } else if (rc != ZV_OK) {
            BASLOG(LOG_LEVEL_ERROR, "rc != ZV_OK");
            goto err;
        }
        ...
        */
        ...
        if (out == NULL) {
            BASLOG(LOG_LEVEL_ERROR, "no enough space for zv_http_out_t");
            exit(1);
        }
        ...
        if (!out->keep_alive) {
            BASLOG(LOG_LEVEL_DEBUG, "no keep_alive! ready to close");
            free(out);
            goto close;
        }
        free(out);
...
static void parse_uri(char *uri, int uri_length, char *filename, char *querystring) {
    ...
    if (uri_length > (SHORTLINE >> 1)) {
        BASLOG(LOG_LEVEL_ERROR, "uri too long: %.*s", uri_length, uri);
        return;
    }
    ...
    BASLOG(LOG_LEVEL_DEBUG, "filename = %s", filename);
    return;
}
...
static void do_error(int fd, char *cause, char *errnum, char *shortmsg, char *longmsg)
{
    ...
    //BASLOG(LOG_LEVEL_DEBUG, "header  = \n %s\n", header);
    rio_writen(fd, header, strlen(header));
    rio_writen(fd, body, strlen(body));
    //BASLOG(LOG_LEVEL_DEBUG, "leave clienterror\n");
    return;
}
...
static void serve_static(int fd, char *filename, size_t filesize, zv_http_out_t *out) {
    ...
    if (n != strlen(header)) {
        BASLOG(LOG_LEVEL_ERROR, "n != strlen(header)");
        goto out; 
    }
